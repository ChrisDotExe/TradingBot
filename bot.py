...
def robinhood_login(r_username, r_password, r_mfa_code=None):
    r.login(r_username, r_password, r_mfa_code)
...
def get_top_gainers(): 
    ...
    for row in soup.find_all('tr', attrs={'class': 'simpTblRow'})[:10]: 
        r_ticker = row.find('td', attrs={'aria-label': 'Symbol'}).text
        tickers.append(r_ticker)
    return tickers
...
def get_reddit_sentiment(ticker):
    ...
    response = requests.get(url, headers=headers)
    soup = BeautifulSoup(response.text, 'html.parser')
    posts = [p.text for p in soup.find_all('h3')[:5]]
    sentiment_score = 0
    for post in posts:
...
def get_stocktwits_sentiment(ticker):
    ...
    response = requests.get(url, headers=headers)
    soup = BeautifulSoup(response.text, 'html.parser')
    messages = [m.text for m in soup.find_all('p')[:5]]
...
def get_cryptopanic_sentiment(ticker):
    url = f'https://cryptopanic.com/news/{ticker.lower()}'
    headers = {'User-Agent': 'Mozilla/5.0'}
    response = requests.get(url, headers=headers)
    soup = BeautifulSoup(response.text, 'html.parser')
    articles = [a.text for a in soup.find_all('h2')[:5]]
...
def get_aggregated_sentiment(ticker, is_crypto=False):
    yahoo_sentiment = get_yahoo_finance_sentiment(ticker)
    reddit_sentiment = get_reddit_sentiment(ticker)
    stocktwits_sentiment = get_stocktwits_sentiment(ticker) if not is_crypto else 0
    cryptopanic_sentiment = get_cryptopanic_sentiment(ticker) if is_crypto else 0
...
def trade_decision(ticker, is_crypto=False):
    sentiment = get_aggregated_sentiment(ticker, is_crypto)
    if is_crypto: crypto_info = r.crypto.get_crypto_quote(ticker)
    price = float(crypto_info['ask_price'])
    allocation = 100  # Example allocation in USD quantity = allocation / price
...
mode = 'backtest'  # Change to 'backtest' for testing

if mode == 'live':
    username = input("Enter Robinhood Username: ")
    password = input("Enter Robinhood Password: ")
    mfa_code = input("Enter MFA Code (if applicable): ")
    robinhood_login(username, password, mfa_code)

# Get top gainers dynamically
for ticker in get_top_gainers():
    is_crypto = ticker.endswith("-USD")
    trade_decision(ticker, is_crypto=is_crypto)

else:
    print("Invalid mode. Choose 'backtest' or 'live'.")
